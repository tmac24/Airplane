...
namespace tinyobj {
	...
	std::string getStringFromFile(std::string fullpath)
	{
		...
		HmsAviPf::Data data = HmsAviPf::CHmsFileUtils::getInstance()->getDataFromFile(fullpath);
		std::string strOut;
		if (!data.isNull())
		{
			char * pData = (char *)data.getBytes();
			strOut.assign(pData, data.getSize());
		}
		return strOut;
#else
...
#define TINYOBJ_SSCANF_BUFFER_SIZE  (4096)
    ...
    static bool exportFaceGroupToShape(
		...
		int nCheckNumber = in_positions.size() / 3;
        ...
        for (size_t i = 0, size = faceGroup.size(); i < size; ++i) {
            ...
            for (size_t k = 2; k < npolys; k++) {
				...
				if (i0.v_idx >= nCheckNumber || i1.v_idx >= nCheckNumber || i2.v_idx >= nCheckNumber)
				{
					continue;
				}
    ...
    std::string MaterialFileReader::operator()(const std::string &matId,
        ...
        std::istringstream matIStream(getStringFromFile(filepath));
        if (!matIStream) {
            std::stringstream ss;
    ...
    std::string LoadObj(std::vector<shape_t> &shapes,
        ...
        std::istringstream ifs(getStringFromFile(filename));
        if (!ifs) {
