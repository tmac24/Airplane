...
CHmsSettingRecordDataPage::CHmsSettingRecordDataPage()
	: m_eAction(RecordFileAction::NoAction)
{
...
CHmsSettingRecordDataPage * CHmsSettingRecordDataPage::Create(const HmsAviPf::Size & size)
{
	CHmsSettingRecordDataPage *ret = new CHmsSettingRecordDataPage();
...
bool CHmsSettingRecordDataPage::init(const HmsAviPf::Size & size)
{
...
void CHmsSettingRecordDataPage::InitComfirmDlg()
{
...
void CHmsSettingRecordDataPage::SetContentCheckBox(const std::vector<RecordItemInfo> & vItemContent)
{
	...
	auto getSelectedBtn = [=](const RecordItemInfo & item)
	{
		...
		if (btn)
		{
			...
			btn->SetCallbackFunc(HMS_CALLBACK_2(CHmsSettingRecordDataPage::OnCheckBoxClick, this, item.strName, item.nParam));
		}
		return btn;
	};

	auto getRadioBtn = [=](const RecordItemInfo & item)
	{
		...
		if (radio)
		{
			...
			radio->SetCallbackFunc(HMS_CALLBACK_2(CHmsSettingRecordDataPage::OnRadioClick, this, item.strName, item.nParam));
		}
		return radio;
	};
...
void CHmsSettingRecordDataPage::OnEnter()
{
...
void CHmsSettingRecordDataPage::AddContentNode(HmsAviPf::CHmsNode * node)
{
	if (node)
	{
...
void CHmsSettingRecordDataPage::RemoveAllContentNode()
{
	m_viewWindow->RemoveAllItem();
	CHmsSettingSubPage::RemoveAllContentNode();
}

void CHmsSettingRecordDataPage::UpdateItem()
{
	m_vItemInfo.clear();
...
bool CHmsSettingRecordDataPage::GetRecordItemInfo(std::vector<RecordItemInfo> & vItemInfo)
{
	RecordItemInfo info;
	info.nParam = 0;
	info.nSize = 0;
	...
	DIR *dir = opendir(g_stringRecordPath.c_str());
	if (nullptr == dir)
	{
	...
	while ((file = readdir(dir)) != NULL)
	{
		...
		if (file->d_type == DT_DIR) {
			...
			if (nullptr != strstr(file->d_name, ".rda"));
			{
				struct stat statbuff;	
				info.strName = file->d_name;
				std::string filePath = g_stringRecordPath + file->d_name;
	...
	closedir(dir);
#else
	std::string strPath = g_stringRecordPath + "*.rda";
...
void CHmsSettingRecordDataPage::OnCheckBoxClick(CHmsUiImage2StateButton * btn, HmsAviPf::HMS_UI_BUTTON_STATE state, const std::string & strName, int nParam)
{
...
void CHmsSettingRecordDataPage::OnClickCopyAll()
{
	...
	else
	{
		...
		m_eAction = RecordFileAction::CopyAll;
#else
		if (m_labelConfirmTitle)
		{
...
void CHmsSettingRecordDataPage::OnClickCopySelected()
{
    if (m_nodeConfirmDlg->IsVisible())
    {
        m_nodeConfirmDlg->SetVisible(false);
    ...
    else
    {
        ...
        m_eAction = RecordFileAction::CopySelected;
#else
        if (m_labelConfirmTitle)
        {
            CHmsLanguageLabel::SetText(m_labelConfirmTitle, "Not support the Action!");
...
void CHmsSettingRecordDataPage::OnClickCleanSelected()
{
	if (m_nodeConfirmDlg->IsVisible())
	{
		m_nodeConfirmDlg->SetVisible(false);
	...
	else
	{
		...
		m_eAction = RecordFileAction::CleanSelected;
	}
}

void CHmsSettingRecordDataPage::OnClickCleanAll()
{
	if (m_nodeConfirmDlg->IsVisible())
	{
		m_nodeConfirmDlg->SetVisible(false);
	...
	else
	{
		...
		m_eAction = RecordFileAction::CleanAll;
	}
}

void CHmsSettingRecordDataPage::OnClickSelectAll()
{
...
void CHmsSettingRecordDataPage::OnClickConfirmDlgCancel()
{
	m_nodeConfirmDlg->SetVisible(false);
	m_eAction = RecordFileAction::NoAction;
}

void CHmsSettingRecordDataPage::OnClickConfirmDlgConfirm()
{
	m_nodeConfirmDlg->SetVisible(false);
	switch (m_eAction)
	{
	case RecordFileAction::NoAction:
		break;
	case RecordFileAction::CopyAll:
		DoCopyAll();
		break;
    case RecordFileAction::CopySelected:
        DoCopySelected();
        break;
	case RecordFileAction::CleanSelected:
		DoCleanSelected();
		break;
	case RecordFileAction::CleanAll:
		DoCleanAll();
		break;
	default:
		break;
...
void CHmsSettingRecordDataPage::DoCopyAll()
{
#if (HMS_TARGET_PLATFORM == HMS_PLATFORM_ANDROID)
	CallAndroidMoveFileToSelectDocTree(g_stringRecordPath, ".rda");
#else

#endif
}

void CHmsSettingRecordDataPage::DoCopySelected()
{
    std::string strNames;
...
#if (HMS_TARGET_PLATFORM == HMS_PLATFORM_ANDROID)
    CallAndroidMoveMutiFilesToSelectDocTree(g_stringRecordPath, strNames, ".rda");
#else

#endif
}

void CHmsSettingRecordDataPage::DoCleanSelected()
{
	int nRemove = 0;
	int nRemoveError = 0;
...
void CHmsSettingRecordDataPage::DoCleanAll()
{
	int nRemove = 0;
	int nRemoveError = 0;
