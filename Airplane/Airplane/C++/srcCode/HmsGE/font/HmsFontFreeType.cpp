...
FT_Library HmsFontFreeType::_FTlibrary;
bool       HmsFontFreeType::_FTInitialized = false;
const int  HmsFontFreeType::DistanceMapSpread = 3;

const char* HmsFontFreeType::_glyphASCII = "\"!#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~¡¢£¤¥¦§¨©ª«¬­®¯°±²³´µ¶·¸¹º»¼½¾¿ÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖ×ØÙÚÛÜÝÞßàáâãäåæçèéêëìíîïðñòóôõö÷øùúûüýþ ";
const char* HmsFontFreeType::_glyphNEHE = "!\"#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~ ";
...
HmsFontFreeType * HmsFontFreeType::create(const std::string &fontName, float fontSize, GlyphCollection glyphs, const char *customGlyphs,bool distanceFieldEnabled /* = false */,int outline /* = 0 */)
{
    HmsFontFreeType *tempFont =  new HmsFontFreeType(distanceFieldEnabled,outline);
...
bool HmsFontFreeType::initFreeType()
{
...
void HmsFontFreeType::shutdownFreeType()
{
...
FT_Library HmsFontFreeType::getFTLibrary()
{
    initFreeType();
    return _FTlibrary;
}

HmsFontFreeType::HmsFontFreeType(bool distanceFieldEnabled /* = false */,int outline /* = 0 */)
: _fontRef(nullptr)
, _stroker(nullptr)
...
, _usedGlyphs(GlyphCollection::ASCII)
{
    if (outline > 0)
    {
        ...
        FT_Stroker_New(HmsFontFreeType::getFTLibrary(), &_stroker);
...
bool HmsFontFreeType::createFontObject(const std::string &fontName, float fontSize)
{
    FT_Face face;
    ...
    else
    {
        ...
        s_cacheFontData[fontName].data = CHmsFileUtils::getInstance()->getDataFromFile(fontName);    
...
HmsFontFreeType::~HmsFontFreeType()
{
    if (_FTInitialized)
    {
        if (_stroker)
...
FontAtlas * HmsFontFreeType::createFontAtlas()
{
    if (_fontAtlas == nullptr)
    {
        _fontAtlas = new STD_NOTHROW FontAtlas(*this);
...
int * HmsFontFreeType::getHorizontalKerningForTextUTF16(const std::u16string& text, int &outNumLetters) const
{
    if (!_fontRef)
        return nullptr;
...
int  HmsFontFreeType::getHorizontalKerningForChars(unsigned short firstChar, unsigned short secondChar) const
{
...
int HmsFontFreeType::getFontAscender() const
{
    return (static_cast<int>(_fontRef->size->metrics.ascender >> 6));
}

unsigned char* HmsFontFreeType::getGlyphBitmap(unsigned short theChar, long &outWidth, long &outHeight, Rect &outRect,int &xAdvance)
{
...
unsigned char * HmsFontFreeType::getGlyphBitmapWithOutline(unsigned short theChar, FT_BBox &bbox)
{   
    unsigned char* ret = nullptr;
...
{
    long pixelAmount = (width + 2 * HmsFontFreeType::DistanceMapSpread) * (height + 2 * HmsFontFreeType::DistanceMapSpread);
    ...
    long outWidth = width + 2 * HmsFontFreeType::DistanceMapSpread;
    for (i = 0; i < width; ++i)
    {
        for (j = 0; j < height; ++j)
        {
            data[j * outWidth + HmsFontFreeType::DistanceMapSpread + i] = img[j * width + i] / 255.0;
        }
    }

    width += 2 * HmsFontFreeType::DistanceMapSpread;
    height += 2 * HmsFontFreeType::DistanceMapSpread;
...
void HmsFontFreeType::renderCharAt(unsigned char *dest,int posX, int posY, unsigned char* bitmap,long bitmapWidth,long bitmapHeight)
{
    int iX = posX;
    int iY = posY;
...
void HmsFontFreeType::setGlyphCollection(GlyphCollection glyphs, const char* customGlyphs /* = nullptr */)
{
    _usedGlyphs = glyphs;
...
const char* HmsFontFreeType::getGlyphCollection() const
{
    const char* glyphCollection = nullptr;
    switch (_usedGlyphs)
    {
    case HmsAviPf::GlyphCollection::DYNAMIC:
        break;
	case HmsAviPf::GlyphCollection::NEHE:
        glyphCollection = _glyphNEHE;
        break;
	case HmsAviPf::GlyphCollection::ASCII:
        glyphCollection = _glyphASCII;
        break;
	case HmsAviPf::GlyphCollection::CUSTOM:
