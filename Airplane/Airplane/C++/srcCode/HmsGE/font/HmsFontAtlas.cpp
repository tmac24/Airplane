...
const char* FontAtlas::CMD_PURGE_FONTATLAS = "__HMS_PURGE_FONTATLAS";
const char* FontAtlas::CMD_RESET_FONTATLAS = "__HMS_RESET_FONTATLAS";

FontAtlas::FontAtlas(HmsFont &theFont) 
: _font(&theFont)
...
, _currLineHeight(0)
{
    ...
    _fontFreeType = dynamic_cast<HmsFontFreeType*>(_font);
    if (_fontFreeType)
    {
        _lineHeight = _font->getFontMaxHeight();
        _fontAscender = _fontFreeType->getFontAscender();
        auto texture = new STD_NOTHROW Texture2D;
        _currentPage = 0;
        _currentPageOrigX = 0;
        _currentPageOrigY = 0;
        _letterEdgeExtend = 2;
        ...
        {
            _letterPadding += 2 * HmsFontFreeType::DistanceMapSpread;    
        }
...
bool FontAtlas::prepareLetterDefinitions(const std::u16string& utf16Text)
{
    ...
    for (auto&& it : codeMapOfNewChar)
    {
        ...
        if (bitmap && bitmapWidth > 0 && bitmapHeight > 0)
        {
            ...
            {
                _currLineHeight = static_cast<int>(bitmapHeight) + _letterPadding + _letterEdgeExtend + 3;
            }
            if (_currentPageOrigX + tempDef.width > CacheTextureWidth)
            {
                ...
                if (_currentPageOrigY + _lineHeight >= CacheTextureHeight)
                {
                    ...
                    memset(_currentPageData, 0, _currentPageDataSize);
                    _currentPage++;
                    auto tex = new STD_NOTHROW Texture2D;
    ...
    //_atlasTextures[_currentPage]->updateWithData(data, CacheTextureWidth, CacheTextureHeight);
    return true;
}
