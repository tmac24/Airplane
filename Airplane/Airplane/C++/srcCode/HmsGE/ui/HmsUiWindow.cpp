...
CHmsUiWindow* CHmsUiWindow::Create(const Size & sizeShow)
{
	CHmsUiWindow *ret = new CHmsUiWindow();
...
bool CHmsUiWindow::Init(const Size & sizeShow)
{
...
void CHmsUiWindow::SetWindowShowRect(const Rect & rect)
{
	if (m_nodeMask)
	{
		m_nodeMask->clear();
...
CHmsUiPanel * CHmsUiWindow::GetUiPanel()
{
	return m_nodePanel;
}

void CHmsUiWindow::AddWindowItem(CHmsNode * node)
{
	if (m_nodePanel && node)
	{
		...
		m_fPanelContentHeight -= c_fItemSpace;
...
void CHmsUiWindow::RegisterAllChildUiEvent()
{
	if (m_nodePanel)
	{
...
bool CHmsUiWindow::OnPressed(const Vec2 & posOrigin)
{
...
void CHmsUiWindow::OnMove(const Vec2 & posOrigin)
{
...
void CHmsUiWindow::OnReleased(const Vec2 & posOrigin)
{
	auto delta = posOrigin - m_posPressStart;
	auto deltaTime = m_time.GetElapsed();
	if (deltaTime < 0.5 && fabsf(delta.y) < 10)
	{
...
void CHmsUiWindow::SetMinimumShowHeight(float fMinimunShowHeight)
{
