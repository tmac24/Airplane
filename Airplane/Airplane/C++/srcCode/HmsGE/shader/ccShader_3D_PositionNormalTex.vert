...
void main(void)
{
    vec4 ePosition = HMS_MVMatrix * a_position;
\n#if (MAX_POINT_LIGHT_NUM > 0)\n
...
\n#if ((MAX_DIRECTIONAL_LIGHT_NUM > 0) || (MAX_POINT_LIGHT_NUM > 0) || (MAX_SPOT_LIGHT_NUM > 0))\n
    v_normal = HMS_NormalMatrix * a_normal;
\n#endif\n

    TextureCoordOut = a_texCoord;
    TextureCoordOut.y = 1.0 - TextureCoordOut.y;
    gl_Position = HMS_PMatrix * ePosition;
}
);
...
void GetPoistionAndNormal(out vec4 position, out vec3 normal)
{
...
void main()
{
    vec4 position;
    vec3 normal;
    GetPoistionAndNormal(position, normal);

    vec4 ePosition = HMS_MVMatrix * position;
\n#if (MAX_POINT_LIGHT_NUM > 0)\n
    for (int i = 0; i < MAX_POINT_LIGHT_NUM; ++i)
    {
        v_vertexToPointLightDirection[i] = u_PointLightSourcePosition[i].xyz- ePosition.xyz;
...
\n#if ((MAX_DIRECTIONAL_LIGHT_NUM > 0) || (MAX_POINT_LIGHT_NUM > 0) || (MAX_SPOT_LIGHT_NUM > 0))\n
    v_normal = HMS_NormalMatrix * normal;
\n#endif\n
