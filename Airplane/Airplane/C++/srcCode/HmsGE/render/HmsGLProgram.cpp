...
// uniform names
const char* GLProgram::UNIFORM_NAME_AMBIENT_COLOR = "HMS_AmbientColor";
const char* GLProgram::UNIFORM_NAME_P_MATRIX = "HMS_PMatrix";
const char* GLProgram::UNIFORM_NAME_MV_MATRIX = "HMS_MVMatrix";
const char* GLProgram::UNIFORM_NAME_MVP_MATRIX  = "HMS_MVPMatrix";
const char* GLProgram::UNIFORM_NAME_NORMAL_MATRIX = "HMS_NormalMatrix";
const char* GLProgram::UNIFORM_NAME_TIME = "HMS_Time";
const char* GLProgram::UNIFORM_NAME_SIN_TIME = "HMS_SinTime";
const char* GLProgram::UNIFORM_NAME_COS_TIME = "HMS_CosTime";
const char* GLProgram::UNIFORM_NAME_RANDOM01 = "HMS_Random01";
const char* GLProgram::UNIFORM_NAME_SAMPLER0 = "HMS_Texture0";
const char* GLProgram::UNIFORM_NAME_SAMPLER1 = "HMS_Texture1";
const char* GLProgram::UNIFORM_NAME_SAMPLER2 = "HMS_Texture2";
const char* GLProgram::UNIFORM_NAME_SAMPLER3 = "HMS_Texture3";
const char* GLProgram::UNIFORM_NAME_ALPHA_TEST_VALUE = "HMS_alpha_value";
...
static const char * HmsAviPf_SHADER_UNIFORMS =
        "uniform mat4 HMS_PMatrix;\n"
        "uniform mat4 HMS_MVMatrix;\n"
        "uniform mat4 HMS_MVPMatrix;\n"
        "uniform mat3 HMS_NormalMatrix;\n"
        "uniform vec4 HMS_Time;\n"
        "uniform vec4 HMS_SinTime;\n"
        "uniform vec4 HMS_CosTime;\n"
        "uniform vec4 HMS_Random01;\n"
        "uniform sampler2D HMS_Texture0;\n"
        "uniform sampler2D HMS_Texture1;\n"
        "uniform sampler2D HMS_Texture2;\n"
        "uniform sampler2D HMS_Texture3;\n"
        "//HMS INCLUDES END\n\n";
...
, _flags()
{
	_DisplayUnit = CHmsAviDisplayUnit::GetInstance();
    HMSASSERT(nullptr != _DisplayUnit, "Director is null when init a GLProgram");
...
bool GLProgram::initWithByteArrays(const GLchar* vShaderByteArray, const GLchar* fShaderByteArray, const std::string& compileTimeDefines)
{
    ...
    if (vShaderByteArray)
    {
        ...
        {
            CCLOG("HmsAviPf: ERROR: Failed to compile vertex shader");
            return false;
       }
    }
    ...
    if (fShaderByteArray)
    {
        ...
        {
            CCLOG("HmsAviPf: ERROR: Failed to compile fragment shader");
            return false;
        }
    }
...
bool GLProgram::initWithFilenames(const std::string& vShaderFilename, const std::string& fShaderFilename, const std::string& compileTimeDefines)
{
    ...
    std::string vertexSource = fileUtils->getStringFromFile(CHmsFileUtils::getInstance()->fullPathForFilename(vShaderFilename));
    std::string fragmentSource = fileUtils->getStringFromFile(CHmsFileUtils::getInstance()->fullPathForFilename(fShaderFilename));
    ...
    if(activeUniforms > 0)
    {
        ...
        if(length > 0)
        {
            ...
            for(int i = 0; i < activeUniforms; ++i)
            {
                ...
                // The ones that start with 'HMS_' are built-ins
                if(strncmp("HMS_", uniformName, 3) != 0) {
...
bool GLProgram::compileShader(GLuint* shader, GLenum type, const GLchar* source, const std::string& convertedDefines)
{
...
#elif (HMS_TARGET_PLATFORM != HMS_PLATFORM_LINUX && HMS_TARGET_PLATFORM != HMS_PLATFORM_MAC)
        (type == GL_VERTEX_SHADER ? "precision highp float;\n precision highp int;\n" : "precision mediump float;\n precision mediump int;\n"),
#else
		(type == GL_VERTEX_SHADER ? "precision highp float;\n precision highp int;\n" : "precision mediump float;\n precision mediump int;\n"),
#endif

        HmsAviPf_SHADER_UNIFORMS,
    ...
    *shader = glCreateShader(type);
    ...
    if (! status)
    {
        ...
        HMSLOG("HmsAviPf: ERROR: Failed to compile shader:\n%s", src);

        if (type == GL_VERTEX_SHADER)
        {
            HMSLOG("HmsAviPf: %s", getVertexShaderLog().c_str());
        }
        else
        {
            HMSLOG("HmsAviPf: %s", getFragmentShaderLog().c_str());
        }
        free(src);
...
bool GLProgram::link()
{
    HMSASSERT(_program != 0, "Cannot link invalid program");
...
#if DEBUG || (HMS_TARGET_PLATFORM == HMS_PLATFORM_WINRT)
    glGetProgramiv(_program, GL_LINK_STATUS, &status);

    if (status == GL_FALSE)
    {
        CCLOG("HmsAviPf: ERROR: Failed to link program: %i", _program);
        GL::deleteProgram(_program);
        _program = 0;
    }
#endif
...
{
    return HmsAviPf::logForOpenGLShader(_vertShader);
}

std::string GLProgram::getFragmentShaderLog() const
{
    return HmsAviPf::logForOpenGLShader(_fragShader);
}
...
{
    HMSASSERT(name != nullptr, "Invalid uniform name" );
    HMSASSERT(_program != 0, "Invalid operation. Cannot get uniform location when program is not initialized");
...
{
    setUniformsForBuiltins(_DisplayUnit->GetMatrix(MATRIX_STACK_TYPE::MATRIX_STACK_MODELVIEW));
}

void GLProgram::setUniformsForBuiltins(const Mat4 &matrixMV)
{
    auto& matrixP = _DisplayUnit->GetMatrix(MATRIX_STACK_TYPE::MATRIX_STACK_PROJECTION);
    ...
    if (_flags.usesTime) {
        ...
        // HmsAviPf doesn't store a high precision time value, so this will have to do.
        // Getting Mach time per frame per shader using time could be extremely expensive.
        float time = _DisplayUnit->GetTotalFrames() * _DisplayUnit->GetAnimationInterval();
