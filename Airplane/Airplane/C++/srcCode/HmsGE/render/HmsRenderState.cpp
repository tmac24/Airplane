...
, _parent(nullptr)
{
    ...
    HMS_SAFE_RETAIN(_state);
}

RenderState::~RenderState()
{
    HMS_SAFE_RELEASE(_state);
}
...
void RenderState::initialize()
{
    if (StateBlock::_defaultState == NULL)
    {
        ...
        HMS_SAFE_RETAIN(StateBlock::_defaultState);
    }
}

void RenderState::finalize()
{
    HMS_SAFE_RELEASE_NULL(StateBlock::_defaultState);
}

bool RenderState::init(RenderState* parent)
{
    HMSASSERT(!_parent, "Cannot reinitialize Render State");
    HMSASSERT(parent, "parent must be non-null");
...
{
    HMSASSERT(renderState, "must be non null");
    ...
    HMS_SAFE_RETAIN(renderState->_texture);
...
{
    auto state = new  RenderState::StateBlock();
    if (state)
    {
        state->autorelease();
    }
...
static RenderState::Blend parseBlend(const std::string& value)
{
    ...
    std::transform(upper.begin(), upper.end(), upper.begin(), ToUpperChar);
    if (upper == "ZERO")
...
{
    HMSASSERT(_defaultState, "_default state not created yet. Cannot be invalidated");
