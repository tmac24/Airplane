...
#define HMS_CLIPPING_NODE_OPENGLES 1
...
#if HMS_CLIPPING_NODE_OPENGLES
static void setProgram(CHmsNode *n, GLProgram *p)
{
    n->SetGLProgram(p);
...
CHmsClippingNode::CHmsClippingNode()
: _stencil(nullptr)
...
CHmsClippingNode* CHmsClippingNode::create()
{
    CHmsClippingNode *ret = new STD_NOTHROW CHmsClippingNode();
    if (ret && ret->Init())
    {
        ret->autorelease();
    }
...
CHmsClippingNode* CHmsClippingNode::create(CHmsNode *pStencil)
{
    CHmsClippingNode *ret = new STD_NOTHROW CHmsClippingNode();
...
bool CHmsClippingNode::Init()
{
    return Init(nullptr);
}

bool CHmsClippingNode::Init(CHmsNode *stencil)
{
    HMS_SAFE_RELEASE(_stencil);
    _stencil = stencil;
    HMS_SAFE_RETAIN(_stencil);
...
void CHmsClippingNode::OnEnter()
{
#if HMS_ENABLE_SCRIPT_BINDING
...
void CHmsClippingNode::drawFullScreenQuadClearStencil()
{
    ...
    //HMS_INCREMENT_GL_DRAWN_BATCHES_AND_VERTICES(1, 4);
    
    director->PopMatrix(MATRIX_STACK_TYPE::MATRIX_STACK_PROJECTION);
    director->PopMatrix(MATRIX_STACK_TYPE::MATRIX_STACK_MODELVIEW);
}

void CHmsClippingNode::Visit(Renderer *renderer, const Mat4 &parentTransform, uint32_t parentFlags)
{
    if (!m_bVisible || !hasContent())
        return;
    ...
    _beforeVisitCmd.func = HMS_CALLBACK_0(CHmsClippingNode::onBeforeVisit, this);
	...
	_stencil->Visit(renderer, m_transformModeView, flags);

	_afterDrawStencilCmd.init(m_globalZOrder);
    _afterDrawStencilCmd.func = HMS_CALLBACK_0(CHmsClippingNode::onAfterDrawStencil, this);
    ...
    _afterVisitCmd.func = HMS_CALLBACK_0(CHmsClippingNode::onAfterVisit, this);
...
// {
//     CHmsNode::setCameraMask(mask, applyChildren);
//     
//     if (_stencil)
//         _stencil->setCameraMask(mask, applyChildren);
// }

CHmsNode* CHmsClippingNode::getStencil() const
{
    return _stencil;
}

void CHmsClippingNode::setStencil(CHmsNode *stencil)
{
    HMS_SAFE_RETAIN(stencil);
    HMS_SAFE_RELEASE(_stencil);
    _stencil = stencil;
}

bool CHmsClippingNode::hasContent() const
{
    return m_vChild.size() > 0;
}

GLfloat CHmsClippingNode::getAlphaThreshold() const
{
    return _alphaThreshold;
}

void CHmsClippingNode::setAlphaThreshold(GLfloat alphaThreshold)
{
    _alphaThreshold = alphaThreshold;
}

bool CHmsClippingNode::isInverted() const
{
    return _inverted;
}

void CHmsClippingNode::setInverted(bool inverted)
{
    _inverted = inverted;
}

void CHmsClippingNode::onBeforeVisit()
{
    ///////////////////////////////////
    // INIT
...
void CHmsClippingNode::onAfterDrawStencil()
{
...
void CHmsClippingNode::onAfterVisit()
{
    ///////////////////////////////////
    // CLEANUP
