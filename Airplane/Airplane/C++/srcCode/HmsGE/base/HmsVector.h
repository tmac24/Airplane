...
public:
    ...
    : _data()
    {
        static_assert(std::is_convertible<T, Ref*>::value, "Invalid Type for HmsAviPf::Vector<T>!");
    }
...
//    }
    ...
    {
        HMSASSERT( index >= 0 && index < size(), "index out of range in getObjectAtIndex()");
        return _data[index];
    }
    ...
    {
        HMSASSERT(object != nullptr, "The object should not be nullptr");
    ...
    {
        HMSASSERT(index >= 0 && index <= size(), "Invalid index!");
        HMSASSERT(object != nullptr, "The object should not be nullptr");
    ...
    void popBack()
    {
        HMSASSERT(!_data.empty(), "no objects added");
    ...
    {
        HMSASSERT(position >= _data.begin() && position < _data.end(), "Invalid position!");
        (*position)->release();
    ...
    {
        HMSASSERT(!_data.empty() && index >=0 && index < size(), "Invalid index!");
    ...
    void swap(T object1, T object2)
    {
        ...
        HMSASSERT(idx1>=0 && idx2>=0, "invalid object index");
    ...
    {
        HMSASSERT(index1 >=0 && index1 < size() && index2 >= 0 && index2 < size(), "Invalid indices");
